...
export default function AuthModal({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-[#16161a] rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        {/* Header */}
                  ...
                  <div
                    ...
                    className={`h-1 flex-1 rounded-full transition-colors ${
                      i <= passwordStrength.score
                        ...
                        : 'bg-gray-200 dark:bg-white/[0.09]'
                    }`}
                  />
                ))}
              </div>
              <span className="text-xs text-gray-500 dark:text-zinc-400">{passwordStrength.label}</span>
            </div>
          )}
        </Field>
